...
export default function Step3_Services() {
  ...
  const { selectedServices, toggleService, setSelectedServices, nextStep, prevStep, goToStep } = useQuoteStore()

  // Default-select DJ on first visit
  useEffect(() => {
    if (selectedServices.length === 0) {
      setSelectedServices(['dj'])
    }
  }, [])
  ...
  const hasCustomizationOptions =
    selectedServices.includes('lighting') ||
    selectedServices.includes('audio') ||
    showSpecialExtras

  const handleNext = () => hasCustomizationOptions ? nextStep() : goToStep(3)
  ...
  return (
    <div className="px-3 py-5 space-y-4">
      ...
      {!hidePricingDuringForm && discountRate > 0 && (
        <div
      ...
      {!hidePricingDuringForm && count >= 1 && count < 4 && (
        <div
